...
var (
	// mockPeer does not use the connection in these tests
	mockPeer = NewPeer(nil)
)
...
func setupEngine() *Engine {
	...
	eng, _ := NewEngine(s, &config.Config{  //nolint:errcheck
...
func TestPing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res := e.Execute(mockPeer, "PING", makeCommand("PING", tt.args...))
...
func TestBasicSetGetDel(t *testing.T) {
	...
	// GET missing key
	res := e.Execute(mockPeer, "GET", makeCommand("GET", "mykey"))
	if res.IsNull != true {
		t.Errorf("expected null for missing key, got %v", res.Type)
	}

	// SET key
	res = e.Execute(mockPeer, "SET", makeCommand("SET", "mykey", "myvalue"))
	...
	// DEL key
	res = e.Execute(mockPeer, "DEL", makeCommand("DEL", "mykey"))
	if res.Integer != 1 {
...
func TestSetTTL(t *testing.T) {
	...
	// SET EX (Seconds)
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_ex", "val", "EX", "1"))

	// Check immediately
	ttl := e.Execute(mockPeer, "TTL", makeCommand("TTL", "k_ex"))
	if ttl.Integer != 1 {
	...
	res := e.Execute(mockPeer, "GET", makeCommand("GET", "k_ex"))
	if res.IsNull != true {
		t.Errorf("key should have expired")
	}

	// SET PX (Milliseconds)
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_px", "val", "PX", "100"))

	pttl := e.Execute(mockPeer, "PTTL", makeCommand("PTTL", "k_px"))
	if pttl.Integer <= 0 || pttl.Integer > 100 {
		t.Errorf("expected PTTL ~100ms, got %d", pttl.Integer)
	}

	time.Sleep(150 * time.Millisecond)
	res = e.Execute(mockPeer, "GET", makeCommand("GET", "k_px"))
	if res.IsNull != true {
...
func TestSetKeepTTL(t *testing.T) {
	...
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_keep", "v1", "EX", "100"))

	// Update value but Keep TTL
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_keep", "v2", "KEEPTTL"))

	val := e.Execute(mockPeer, "GET", makeCommand("GET", "k_keep"))
	if string(val.String) != "v2" {
		t.Errorf("KEEPTTL value not updated")
	}

	// Verify TTL is still approx 100
	ttl := e.Execute(mockPeer, "TTL", makeCommand("TTL", "k_keep"))
	...
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_new_keep", "v1", "KEEPTTL"))
	ttl = e.Execute(mockPeer, "TTL", makeCommand("TTL", "k_new_keep"))
	if ttl.Integer != -1 {
...
func TestSetTimestamps(t *testing.T) {
	...
	e.Execute(mockPeer, "SET", makeCommand("SET", "k_exat", "v", "EXAT", futureStr))

	ttl := e.Execute(mockPeer, "TTL", makeCommand("TTL", "k_exat"))
...
func TestTTL_PTTL_Codes(t *testing.T) {
	...
	// Missing Key -> -2
	res := e.Execute(mockPeer, "TTL", makeCommand("TTL", "missing"))
	if res.Integer != -2 {
		t.Errorf("expected -2 for missing key, got %d", res.Integer)
	}

	// Persistent Key -> -1
	e.Execute(mockPeer, "SET", makeCommand("SET", "persistent", "val"))
	res = e.Execute(mockPeer, "TTL", makeCommand("TTL", "persistent"))
	if res.Integer != -1 {
		t.Errorf("expected -1 for persistent key, got %d", res.Integer)
	}
	res = e.Execute(mockPeer, "PTTL", makeCommand("PTTL", "persistent"))
	if res.Integer != -1 {
...
func TestSetSyntaxErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res := e.Execute(mockPeer, "SET", makeCommand("SET", tt.args...))
